...
import { int2str, str2int1, str2int2, isLtLL, isGtLL, addint1, addint2 } from './int64'
...
function RandomIncreasing() {
  ...
  const min_num = 1, max_num = 300000;
  ...
  useEffect(() => {
    ...
    else if (isStrict && (upper1 === lower1) && upper2 - lower2 + 1 < num) {
      setIllegal(true);
    }
    else if (!isStrict && isGtLL(lower1, lower2, upper1, upper2)) {
      setIllegal(true);
    }
    else {
      setIllegal(false);
  ...
  }, [num, lower1, lower2, upper1, upper2, isStrict]);
	...
	return (
      ...
      <div className="form-check">
        <input 
          ...
          checked={isStrict} 
          onChange={() => setIsStrict(prevState => !prevState)} 
        />
...
export default RandomIncreasing;